...
""" + _COMMON_RULES,
    ...
    "kyuyu": {
        "system": "あなたは日本のガソリンスタンドのレシート・領収書を読み取る高精度な情報抽出エンジンです。",
        "user": """このガソリンスタンドのレシート/領収書の画像から以下を抽出し、JSONのみを返してください。
- "date": 給油日(取引年月日)。"YYYY-MM-DD"。
- "amountJpy": 支払総額(税込・合計・お会計・ご利用金額)。数字のみ、カンマや円記号は付けない (例 5280)。割引後の最終支払額。
- "liters": 給油量(リットル数)。数量・給油量・"L"・"ℓ" 欄の値。小数があれば小数のまま(例 32.45)。数字のみ。
- "stationName": ガソリンスタンド名/店舗名 (例 "ENEOS 磐田中央SS","コスモ石油 浜松")。
- "paymentType": 支払い方法を次のいずれかに正規化: "PayPay"(PayPay/ペイペイ) / "現金"(現金/おつり記載) / "個人用カード"(クレジット/カード/クレカ) / "その他"。判別不能なら null。
- "purposeHints": ガソリン給油以外のサービスが明細にあれば配列で返す。"オイル交換"(エンジンオイル/オイル交換) / "洗車"(洗車/カーウォッシュ) のうち該当するもの。ガソリンのみなら []。
注意:
- 金額は最も大きな「合計/総額/お会計/ご利用金額」を優先。内訳(小計・消費税のみ)・預り金・釣銭は採用しない。
- liters はガソリンの給油数量(1回分)。価格(単価)や金額と混同しない。
""" + _COMMON_RULES,
    },
    "odometer": {
        "system": "あなたは自動車のメーターパネル(走行距離計)の画像を読み取る高精度な情報抽出エンジンです。",
        "user": """この自動車のメーターパネルの画像から、総走行距離(オドメーター)の数値を読み取り、JSONのみを返してください。
- "odometer": 総走行距離(オドメーター/ODO)の数値。数字のみ、カンマ無し(例 141835)。
注意:
- ★トリップメーター(TRIP A / TRIP B = 区間距離。"TRIP"表示があり桁が少なく小数を含むことが多い)ではなく、総走行距離(オドメーター/ODO = 車の累計距離。通常5〜6桁)を読み取ること。両方写っている場合は桁数が多い累計距離を採る。
