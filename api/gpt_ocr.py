...
- "zairyuKigen": 在留期間(満了日)。"YYYY-MM-DD" 形式 (西暦)。
    ※在留期間欄が「****年**月**日」「無期限」等(永住者・特別永住者・高度専門職2号など)で満了日が無い場合は、
      カード下部の「このカードは ○年○月○日 まで有効」(PERIOD OF VALIDITY OF THIS CARD)の日付を入れること。
      **重要な手がかり: このカード有効期限の「月・日」は交付年月日(DATE OF ISSUE)と同じで、年だけが7年後**になる
      (例: 交付 2025-09-22 なら 有効期限は 2032-09-22)。
      カードには「許可年月日」(別の日付)も併記されており、その日の数字と混同しやすいので、
      **必ず交付年月日の月日を基準にして有効期限の月日を確定すること**。交付年月日や許可年月日そのものを在留期限にしてはいけない。
