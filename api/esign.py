...
def _signed_page(rec: Dict[str, Any], token: str) -> str:
    ...
    return (
        ...
        '.note{font-size:11.5px;color:#888;line-height:1.6;margin-top:6px}'
        # 画面下に常駐する「終了」バー (最下部まで行かなくても押せる・残り時間を表示)
        '#endbar{position:fixed;left:0;right:0;bottom:0;z-index:20;background:rgba(255,255,255,.96);border-top:2px solid #c62828;'
        'box-shadow:0 -4px 14px rgba(0,0,0,.12);padding:10px 12px calc(10px + env(safe-area-inset-bottom));}'
        '#endbar .in{max-width:560px;margin:0 auto;display:flex;gap:10px;align-items:center}'
        '#endbar .btn.red{flex:1;width:auto;padding:13px 10px;font-size:15px}'
        '#endbar .cnt{font-size:11px;color:#777;white-space:nowrap;text-align:right;line-height:1.4}'
        'body{padding-bottom:110px}'
        '.step{display:inline-block;background:#1565c0;color:#fff;border-radius:50%;width:22px;height:22px;line-height:22px;text-align:center;font-size:12px;font-weight:700;margin-right:6px}'
        '</style></head><body>'
        ...
        '<span style="color:#999">Toque no campo, digite o e-mail e toque em enviar.</span></div>'
        '<div class="row" style="margin-top:6px">'
        f'<input type="email" id="mailAddr" placeholder="メールアドレス / e-mail" value="{emp_email}" autocomplete="email" inputmode="email" enterkeyhint="send" onkeydown="if(event.key===\'Enter\'){{event.preventDefault();sendMail();}}">'
        '<button class="btn" onclick="sendMail()" id="mailBtn">📧 メールで受け取る / Enviar</button></div>'
        '<div id="mailMsg" class="note"></div>'
        '<div class="note">控えのPDF（30日間有効のリンク付き）をメールで送ります / Enviar a cópia por e-mail (válido por 30 dias).</div>'
        ...
        '<h3>🔒 終了 / Encerrar</h3>'
        '<div class="note">受け取りが済んだら、画面下の赤いボタン「終了して画面を消す」を押してください。画面と履歴からこの書類を消します。3分間操作がないと自動で消えます。<br>'
        'Ao terminar, toque no botão vermelho "Encerrar" na parte inferior. A tela será apagada após 3 min sem uso.</div>'
        '</div>'
        # 常駐の終了バー (残り時間つき)
        '<div id="endbar"><div class="in">'
        '<button class="btn red" onclick="finish()">🔒 終了して画面を消す / Encerrar</button>'
        '<div class="cnt">自動消去まで<br><b id="cnt">3:00</b></div>'
        '</div></div>'
        ...
        'async function sendMail(){var inp=document.getElementById("mailAddr");var a=inp.value.trim();var m=document.getElementById("mailMsg");var b=document.getElementById("mailBtn");'
        # 未入力/不正なら入力欄へフォーカス (ボタンタップ=ユーザー操作なので iOS でもキーボードが開く)
        'if(!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(a)){m.className="err";m.textContent=a?"メールアドレスを確認してください / Verifique o e-mail":"メールアドレスを入力してください / Digite o e-mail";try{inp.focus();inp.select();}catch(_){}return;}'
        ...
        # 無操作3分で自動消去。残り時間を終了バーに表示 (操作でリセット)
        'var idle=null,left=180,tick=null;function touch(){clearTimeout(idle);idle=setTimeout(finish,180000);left=180;}touch();'
        'tick=setInterval(function(){left=Math.max(0,left-1);var e=document.getElementById("cnt");if(e){e.textContent=Math.floor(left/60)+":"+("0"+(left%60)).slice(-2);}},1000);'
        '["click","touchstart","keydown","scroll","input"].forEach(function(ev){document.addEventListener(ev,touch,{passive:true});});'
        '</script></body></html>'
    )
